...
class Spe extends Base {
    ...
    private readonly speSpecialFields: { name: string, transform: ($div: any) => any }[] = [
        {
            name: 'mark',
            transform: ($div: any) => {
        ...
        {
            name: 'employee',
            transform: ($div: any) => $div.addClass('area-modal').attr('contenteditable', 'false')
        },
        {
            name: 'subDivision',
            transform: ($div: any) => $div.addClass('area-modal').attr('contenteditable', 'false')
    ...
    private async openDocument(event: Event): Promise<void> {
        ...
        $(document).off('change', '#fileInput').on('change', '#fileInput', (e) => this.addFileToDocument(e as unknown as Event, currentSpeId));
