...
class NtDocuments extends Base {
    ...
    private openDocument = async (event: Event): Promise<void> => {
        ...
        dialog.off('change', '#reloadFileInput').on('change', '#reloadFileInput', (reloadEvent: Event) => {
            const currentInput = reloadEvent.currentTarget as HTMLInputElement;
            if (!currentInput.files || currentInput.files.length === 0) return;
    ...
    private openReferences = async (event: Event): Promise<void> => {
        ...
        dialog.off('click', '.download-inn').on('click', '.download-inn', (e: Event) => {
